...
namespace Microsoft.ProjectOxford.Vision
{
    ...
    public class VisionServiceClient : IVisionServiceClient
    {
        ...
        {
            var requestUrl = string.Format("{0}/{1}?visualFeatures={2}&{3}={4}", ServiceHost, AnalyzeQuery, this.VisualFeaturesToString(visualFeatures), SubscriptionKeyName, this.subscriptionKey);
        ...
        {
            var requestUrl = string.Format("{0}/thumbnails?width={1}&height={2}&smartCropping={3}&{4}={5}", ServiceHost, width, height, smartCropping, SubscriptionKeyName, this.subscriptionKey);
            var request = WebRequest.Create(requestUrl);
        ...
        {
            var requestUrl = string.Format("{0}/ocr?language={1}&detectOrientation={2}&{3}={4}", ServiceHost, languageCode, detectOrientation, SubscriptionKeyName, this.subscriptionKey);
            var request = WebRequest.Create(requestUrl);
